...
class _ScrollTimeSeriesSplitLoader:
...
class _ScrollTimeSeriesSplitLoader_: 
    def __init__(self, data: pd.DataFrame, n_splits: int, test_size: int = None):
        self.data = data
        self.data_size = data.shape[0]
        if self.data_size == 0:
            raise ValueError("DataFrame is empty. Cannot initialize ScrollTimeSeriesSplitLoader_")
    ...
    def __count_train_test_sizes(self, test_size: int):
        ...
        """
        if self.n_splits <= 0:
            raise ValueError(f"[n_splits] must be greater than 0, but got {self.n_splits}")
        if test_size == None:
...
class _ExpandingTimeSeriesSplitLoader:
...
class _ExpandingTimeSeriesSplitLoader_:
    ...
    def __init__(self, data: pd.DataFrame, n_splits: int, test_size: int = None):
        ...
        self.data_size = data.shape[0]
        if self.data_size == 0:
            raise ValueError("DataFrame is empty. Cannot initialize ExpandingTimeSeriesSplitLoader_")
        self.n_splits = n_splits
        self.__count_train_test_sizes(test_size)
        self.fold_num = 0
