...
def _validate_json_keys(required_keys: dict, keys: dict) -> None:
    """
...
def _validate_json_values(key: str, dictionary, checking_type=None) -> None:
    """
    ...
    if checking_type != None:
        if type(val) != checking_type:
            raise TypeError(f"Incorrect [{key}] type. Required type ({checking_type}).")
